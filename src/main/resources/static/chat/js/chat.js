...
var app = new Vue({
    ...
    methods: {
        ...
        appendmsg(mstype, revive, send, text) {
            var that = this;
            that.listmessage.push({msgtype: mstype, receiver: revive, sender: send, sendtext: text});
            setTimeout(function () {
...
function setMessageInnerHTML(innerHTML) {
    ...
    if (msgs[0] === actuserid) {
        ...
        layer.msg('THE USER IS NOT ONLINE', {
            time: 1500,
            icon: 0,
            offset: '50px'
        });
...
function sendaudio(message) {
    if (actuserid == null) {
        layer.msg("PLEASE CHOOSE A USER TO CHAT WITH", {
            time: 2500,
            icon: 2,
            offset: "300px"
        });
